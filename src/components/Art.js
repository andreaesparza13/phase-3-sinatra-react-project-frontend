...
const Art = ({id, title, medium, description, image_url, artist, handleClick }) => {
    return (
        <div className="rounded-lg shadow-lg bg-white max-w-sm p-6 m-6 ml-auto mr-auto">
            ...
            <button id={id} onClick={handleClick} type="button" className="inline-block px-6 py-2.5 bg-white-600 text-gray-900 font-medium text-sm leading-tight uppercase">Explore Artist</button>
            </Link>
            </div>
        </div>
    )
